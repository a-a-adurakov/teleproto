...
class MTProtoState {
    ...
    async _calcKey(authKey, msgKey, client) {
        ...
        const [sha256a, sha256b] = await Promise.all([
            (0, Helpers_2.sha256)(Buffer.concat([msgKey, authKey.slice(x, x + 36)])),
            (0, Helpers_2.sha256)(Buffer.concat([authKey.slice(x + 40, x + 76), msgKey])),
        ]);
    ...
    async encryptMessageData(data) {
        ...
        const s = (0, Helpers_2.toSignedLittleBuffer)(this.salt, 8);
        const i = (0, Helpers_2.toSignedLittleBuffer)(this.id, 8);
        data = Buffer.concat([Buffer.concat([s, i]), data]);
        const padding = (0, Helpers_1.generateRandomBytes)((0, Helpers_1.mod)(-(data.length + 12), 16) + 12);
        // Being substr(what, offset, length); x = 0 for client
        // "msg_key_large = SHA256(substr(auth_key, 88+x, 32) + pt + padding)"
        const msgKeyLarge = await (0, Helpers_2.sha256)(Buffer.concat([authKey.slice(88, 88 + 32), data, padding]));
    ...
    async decryptMessageData(body) {
        ...
        const ourKey = await (0, Helpers_2.sha256)(Buffer.concat([authKey.slice(96, 96 + 32), body]));
