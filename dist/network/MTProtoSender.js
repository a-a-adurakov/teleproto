...
class MTProtoSender {
    ...
    send(request) {
        if (this._needsInitConnection && this._isApiRequest(request)) {
            const initConnection = new tl_1.Api.InitConnection({
        ...
        while (this._userConnected && !this.isReconnecting) {
            ...
            try {
                ...
                if (!this.userDisconnected) {
                    this._log.debug(`Connection closed while sending data ${e}`, e);
                    this.reconnect();
                }
                this._sendLoopHandle = undefined;
                return;
        ...
        while (this._userConnected && !this.isReconnecting) {
            ...
            try {
                ...
                if (e instanceof errors_1.TypeNotFoundError) {
                    ...
                    if (e.code === 444) {
                        const otherDc = (_c = (_b = this._client._config) === null || _b === void 0 ? void 0 : _b.dcOptions) === null || _c === void 0 ? void 0 : _c.find((dc) => !dc.cdn && !dc.tcpoOnly && dc.id !== this._dcId);
                        if (otherDc) {
                            this._log.warn(`Transport 444 for dc ${this._dcId}, trying dc ${otherDc.id}`);
                            throw new errors_1.PhoneMigrateError({ request: undefined, capture: otherDc.id });
                        }
    ...
    async _processMessage(message) {
        ...
        this._log.debug(`[MSG] Processing msgId=${message.msgId} type=${message.obj.className} constructorId=${message.obj.CONSTRUCTOR_ID}`);
    ...
    async _handleContainer(message) {
        this._log.debug(`[CONTAINER] msgId=${message.msgId} contains ${message.obj.messages.length} messages`);
    ...
    async _handleUpdate(message) {
        ...
        this._log.debug(`[UPDATE] msgId=${message.msgId} type=${message.obj.className}` +
            (message.obj.updates
                ? ` innerUpdates=${message.obj.updates.length}:[${message.obj.updates.map((u) => u.className).join(",")}]`
                : ""));
        if (this._updateCallback) {
    ...
    async _handleBadServerSalt(message) {
        ...
        (_a = this._dcenter) === null || _a === void 0 ? void 0 : _a.updateSalt(badSalt.newServerSalt);
    ...
    async _handleNewSessionCreated(message) {
        ...
        (_a = this._dcenter) === null || _a === void 0 ? void 0 : _a.updateSalt(message.obj.serverSalt);
        this._needsInitConnection = true;
    }
    /**
    ...
    async _reconnect() {
        ...
        if (toQueryStatus.length > 0) {
            const msgIds = toQueryStatus.filter((s) => s.msgId != undefined).map((s) => s.msgId);
            if (msgIds.length > 0) {
                this._log.debug(`Querying status of ${msgIds.length} acknowledged requests`);
                this.send(new tl_1.Api.MsgsStateReq({ msgIds }));
            }
